...
`k_target`은 영등포 앵커에서 역산해 **3.3 veh/lane-km**. (86대 / 26.0 lane-km. 교과서
임계밀도 25보다 한참 낮은데, 정체가 **상위 10% 엣지에 75% 몰리기** 때문에 네트워크 평균으로는
훨씬 낮은 값에서 이미 병목이 터진다.)

T_ff는 아무 기준 통행량(예 1,000)으로 한 번 생성해 얻으면 된다 — 자유류에서는 전부 선형이라
수준과 무관하다.

**⚠️ 첫 검증 과제:** `k_target=3.3`은 **영등포 한 구역에서만** 뽑은 값이다.
이 공식이 안양·의왕에 대해 내놓는 예측은 **N\* ≈ 6,500**. 스윕으로 맞는지 확인하는 게
이 계획의 첫 작업이다. 안 맞으면 `k_target`이 구역 특성(집중도)의 함수라는 뜻이고,
그 경우 **상위 10% 엣지 점유율**을 보정항으로 넣는다(이 값도 duarouter 정적 배정에서
시뮬 없이 나온다).

**2단계 — γ 보정 (시뮬 2~3회, ~3~6분)**

시드에서 출발해 "생겼다 풀린다" 구간으로 이분 탐색. 판정 기준(§5-B):

| 지표 | 목표 |
|---|---|
| 피크 정지 비율 | 0.4 ~ 0.85 |
| 종료시 잔류 | 도착 차량의 5% 이하 (**필수**) |
| 순간이동 | 도착 차량의 3% 이하 |

**반드시 아래에서 위로 브래킷할 것.** 교착된 실행은 느리고(차가 안 빠져 끝까지 굴러감)
얻는 정보도 없다. 시드 → 실패하면 ×0.8, 통과하면 ×1.2 식으로 위로.
...
- **k_target 앵커가 1점뿐**이다. 2구역 검증 후에도 도심 간선이 많은 구역(영등포 net엔
  primary/secondary가 **아예 없다** — residential 15.3km + tertiary 5.6km뿐)에서는 다시 봐야 한다.
...
4. **N\* 자동 산정** — 계획 수립 완료(§5-C). 3단계: 해석적 시드(시뮬 0회) → γ 보정
   (시뮬 2~3회) → net 해시로 캐시. **첫 작업은 `k_target=3.3` 검증** — 이 공식이
   안양·의왕에 대해 N\* ≈ 6,500을 예측하니 스윕으로 맞는지 확인할 것.
