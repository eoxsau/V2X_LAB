...
| **`demand/calibration.py`** | **§5-C N\* 자동 산정** | **`cached_nstar(net, out_dir, profile) → NStarResult`** (구역·창당 1회, net 해시 캐시); `estimate_nstar_seed()` (시뮬 0회 해석적 시드); `calibrate_nstar()` (γ 이분 보정); `network_lane_km()`; `peak_rate_per_hour()`. 상수 `K_TARGET=3.17`, `DEFAULT_SAFETY_FACTOR=0.9` |
