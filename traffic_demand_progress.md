...
5. 배정 (OD→도로별 교통량) .......... 🔶 관통은 됨. 수요 손실 절반 해결, 절반 남음(§5-A)
6. SUMO 운영설정 + 시간프로파일 주입 . ⬜ 다음
7. 파이프라인 통합 ................. ⬜ 다음
   (RL은 7단계 이후 최종 층으로. 지금은 손대지 않음 — 친구가 재설계 중)
```

**지금 위치: 5단계 end-to-end 관통 성공, 최종 생존율 45.4%.** 수요 손실 두 군데 중
앞단(존→TAZ, 26.5%→93.6%)은 해결했고 **뒷단(duarouter 48.7%)이 다음 1순위**다. §5-A 참조.
...
| `demand/assignment.py` | §7·§8 배정 준비 | `build_taz(net_file, cell, ref_lat)`; `write_taz_xml()`; `write_od_o_format(flows, zone_taz, path, begin_h, end_h)`; **`map_zones_to_taz(zones, taz, cell_size_m, max_reassign_m=900)`** (도로 없는 존 → 최근접 도로존 재배정); **`taz_mapping_summary()`** (자기셀/재배정/버림 진단) |
...
### 현재 상태 — 관통은 되지만 **최종 생존율 45.4%**

```
total_trips 5000
   → OD 파일 적재    4681  (93.6%)   ← 재배정 적용 후. 적용 전엔 1326(26.5%)
   → od2trips trip   4670
   → duarouter 차량  2272  (45.4%)   ← 🔴 여기서 절반이 증발. 미해결
```

| 단계 | 결과 |
|---|---|
| netconvert (한글 경로) | ✅ 신호 11개 |
| 건물 로드 | ✅ 3,577동 / 연면적 4.90km² / 층수 결측 0동 (`POSTGIS_ENABLED=false` 필요, §4) |
| build_zones | ✅ 존 47개, 질량 max/중앙 2.9배 |
| radiation OD | ✅ 흐름 2,162개, 통행거리 중앙 949m |
| TAZ + OD | ✅ **93.6% 적재** (재배정으로 해결. 버려진 존 0개) |
| od2trips | ✅ trip 4,670개 (**ASCII 스테이징 필수** — §7) |
| duarouter | 🔴 **2,272대 (48.7%)** — 2,398대가 "경로 없음" |

### ✅ 해결됨: 도로 없는 존의 수요 (커밋 `e46471e`)
...
### 🔴 미해결: duarouter 라우팅률 48.7% — **다음 작업 1순위**

**원인은 규명 끝났다. 승용차 전용 그래프가 쪼개져 있다.**

| | 값 |
|---|---|
| 승용차 통행가능 엣지 | 498개 |
| 최대 강연결성분(SCC) | **448개 (90%)** |
| 고립 성분에 흩어진 엣지 | **50개** (성분 크기 14 / 6 / 2 / 2 / 1 …, 총 31개 성분) |

`build_taz`가 **이 고립 엣지들도 TAZ에 담기 때문에**, od2trips가 그걸 출발지·도착지로 뽑고
duarouter가 경로를 못 찾는다. 실패 상위 목적지 엣지가 정확히 그 섬들이었다 —
`517043458#*`(14짜리 섬, 815건) · `278157943#0`(6짜리 섬) · `517254646#3`(단독).
**엣지의 10%가 실패의 51%를 만든다** (TAZ가 길이 가중이라 긴 섬 도로가 자주 뽑힘).
...
**→ 제안 해법:** `build_taz`가 **최대 승용차 SCC에 속한 엣지만** TAZ에 담도록 필터.
이미 만든 재배정과 자연스럽게 맞물린다 — 고립 도로밖에 없던 존은 TAZ가 없어지고,
재배정이 알아서 가장 가까운 **연결된** 존으로 보낸다. 두 변경이 서로를 보완한다.

### 남은 작은 손실: self-loop 319통행

재배정으로 A존이 B셀에 합쳐지면 A→B 통행이 `o==d`가 되고, `write_od_o_format`이 이를
건너뛴다(radiation이 i==j를 제외하는 규칙을 그대로 따름). od2trips는 **같은 TAZ 안에서도
서로 다른 source/sink 엣지를 뽑을 수 있으므로 살리는 게 가능**하다. 살릴지 판단 필요(§6).
...
- 🔴 **`build_taz` 고립 성분 필터** — 최대 승용차 SCC만 담을지 (§5-A). **라우팅률 48.7%의 원인.
  다음 작업 1순위.** N\* 튜닝의 선결조건이기도 하다(생존율이 흔들리면 뭘 튜닝하는지 알 수 없음).
- **self-loop 319통행 살릴지** — 재배정이 만든 `o==d`를 `write_od_o_format`이 건너뛴다.
  od2trips는 같은 TAZ 안에서도 다른 엣지를 뽑을 수 있어 살리기 가능 (§5-A).
...
**먼저 §5-A를 읽을 것.** 파이프라인은 관통되고 앞단 손실은 해결됐지만, **duarouter에서 절반이
사라진다(최종 생존율 45.4%).** 원인은 이미 규명돼 있으니 바로 고치면 된다.

**0. 현재 상태 재현부터** — 아무것도 고치기 전에 한 번 돌려서 기준선을 확인:
```bash
backend/.venv/Scripts/python.exe scripts/smoke_demand_pipeline.py
```
기대값: 존 47개 / OD적재 93.6% / trip 4670 / 차량 2272 / 최종 45.4%.

1. 🔴 **`build_taz` 고립 성분 필터** — 최대 승용차 SCC 밖 엣지를 TAZ에서 제외.
   라우팅률 48.7%의 직접 원인이고 재배정과 맞물려 동작한다(§5-A). **이것부터.**
   고친 뒤 스모크를 다시 돌려 라우팅률이 90%대로 오르는지 확인.
2. **self-loop 319통행 처리 결정** (§6) — 살릴지 말지. 1번 이후 숫자를 보고 판단해도 됨.
3. **오케스트레이터 모듈화** — `smoke_demand_pipeline.py`의 흐름을 `backend/app/services/demand/`
   아래 정식 모듈로. 모듈 3개는 **아직 앱 어디에서도 import되지 않는다**(고아 상태).
   od2trips만 ASCII 스테이징(§7), 건물은 parquet 직접(§4).
4. **시간대 OD 슬라이스** — `write_od_o_format`이 이미 `begin_h/end_h/factor`를 받으므로
   24h 곡선으로 **반복 호출**만 하면 됨.
5. **동적 SUMO** — 예열 → 피크 스냅샷 → 엣지별 교통량 → 배치 수요.

> 참고 명령/데이터는 §4·§5·§7에 다 있음. 모듈은 `backend/app/services/demand/`에 준비됨.
> **N\* 레벨·λ·시뮬레이션 창 길이 튜닝은 1번 이후로 미룰 것** — 생존율이 45%에서 흔들리는
> 동안에는 무엇을 튜닝하는 건지 알 수 없다.
