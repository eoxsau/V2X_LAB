...
## 5-A. 스모크 테스트 결과 (2026-07-27) — **읽고 시작할 것**

파이프라인을 처음으로 **end-to-end 관통**시켜 봤다(영등포 `area-0baecbba`, 3.3km², 건물 3,577동,
존 47개, 임시 `total_trips=5000`). 결론: **흐름은 성립하지만 통행량의 절반 이상이 조용히 사라진다.**

> ⚠️ **모듈 3개(`grid_mass`/`radiation`/`assignment`)는 아직 아무 곳에서도 import되지 않는다.**
> 리포에 오케스트레이터가 없다. 지금까지 검증은 전부 일회성 스크립트였다.
...
| 건물 로드 | ✅ 3,577동 / 연면적 4.90km² / 층수 결측 0동 — **단 `POSTGIS_ENABLED=false` 필요**(§4 경고) |
| build_zones | ✅ 존 47개, 질량 max/중앙 2.9배 |
| radiation OD | ✅ 흐름 2,162개, 통행거리 중앙 949m |
| TAZ + OD 작성 | ⚠️ **통행량 26.5%만 생존** (5000 → 1326) |
| od2trips | ❌ 한글 경로 실패 → ASCII로 복사 시 ✅ trip 1,322개 |
| duarouter | ⬜ 미도달 |

### 🔴 최대 문제: 질량 48.7%가 배정 전에 버려짐

존 47개 중 **23개만** TAZ에 매핑된다(`map_zones_to_taz`가 도로 없는 셀을 `None`으로 버림).
원인을 분해하면:

| 원인 | 존 | 질량 | 성격 |
|---|---|---|---|
| 승용차 엣지 있음 (생존) | 26 | 58.2% | — |
| **네트워크에 엣지가 아예 없음** | 18 | **36.2%** | 6개는 bbox 경계 밖(경계 효과), 12개는 bbox 안 |
| 보행자 전용 엣지만 있음 | 3 | 5.6% | `allows("passenger")` 필터 |
| *(위 중) `build_taz` 중점 규칙 탓* | *3* | *6.9%* | **회복 가능** — 아래 참조 |

- **존↔TAZ 정렬은 정상이다.** 셀 인덱스 범위가 정확히 겹친다(ix 37326~37331 양쪽 동일).
  즉 `ref_lat` 정합 문제가 **아니다**. 진짜로 도로가 없는 셀들이다.
- **`build_taz`의 중점 규칙이 일부 원인**([`assignment.py:48`](backend/app/services/demand/assignment.py)):
  엣지를 **shape 중점 1개**로만 셀에 배정하므로, 셀을 가로지르는 긴 엣지가 중점이 옆 셀에 있으면
  그 셀은 "도로 없음"이 된다. 엣지 shape의 **모든 정점**이 닿는 셀에 배정하도록 바꾸면
  질량 생존율 **51.3% → 58.2%** (존 23 → 26개). 값싼 개선이지만 **근본 해결은 아니다.**

**→ 핵심 결정거리:** 도로 없는 셀의 수요를 **버릴지, 가장 가까운 도로 있는 존으로 재배정할지.**
지금은 말없이 버려서 `total_trips`를 5000으로 넣어도 실제 배정은 1326이 된다. N\* 튜닝(§6)을
하기 전에 이걸 정해야 한다 — 안 그러면 **무엇을 튜닝하는지 알 수 없다.**
...
- 🔴 **도로 없는 존의 수요 처리** — 버릴지 / 최근접 도로존으로 재배정할지 (§5-A). **N\* 튜닝의 선결조건.**
- **`build_taz` 중점 규칙 개선** — 엣지 전체 shape 기준으로 바꿀지(질량 +6.9%p). 값싼 개선.
- **`postgis_available()` 수정 여부** — 실제 연결을 확인하도록 고칠지(앱 전체 영향) / 수요
  파이프라인만 parquet 직접 읽게 할지(국소적). §4 참조.
...
- **⚠️ 한글 경로 문제 — 도구마다 다름 (2026-07-27 실측으로 정정):**
  ...
  | 도구 | 한글 경로 | 비고 |
  |---|---|---|
  | `netconvert` | ✅ **정상 동작** | `C:\Users\최동혁\...`에서 rc=0. 기존 문서의 "netconvert도 동일"은 **틀림** |
  | `od2trips` | ❌ **실패** | `Error: Could not open '...\smoke.od.txt'` — 파일 내용은 동일한데 ASCII 경로로 복사하면 즉시 성공(trip 1322개) |

  → 오케스트레이터는 **od2trips 단계만 ASCII 임시 디렉터리에 스테이징**하면 된다.
  전체를 ASCII로 옮길 필요 없음. (`duarouter`·`sumo`는 아직 미검증 — 6단계에서 확인할 것.)
...
**먼저 §5-A(스모크 테스트 결과)를 읽을 것.** 파이프라인은 관통되지만 통행량의 73%가 사라진다.

1. **수요 손실 정책 결정** (§6 첫 항목) — 도로 없는 존을 버릴지 재배정할지. 이걸 정하기 전에
   N\* 튜닝은 의미가 없다.
2. **오케스트레이터 작성** — 리포에 없는 조립 라인. od2trips 단계만 **ASCII 임시 디렉터리**에
   스테이징(§7), 건물은 **parquet 직접**(§4 PostGIS 함정 회피).
3. **시간대 OD 슬라이스** — `write_od_o_format`이 이미 `begin_h/end_h/factor`를 받으므로
   24h 곡선으로 **반복 호출**만 하면 됨.
4. **동적 SUMO** — 예열 → 피크 스냅샷 → 엣지별 교통량 → 배치 수요.

> 참고 명령/데이터는 §4·§5·§7에 다 있음. 모듈은 `backend/app/services/demand/` 에 준비됨.
> 스모크 테스트 스크립트는 scratchpad(세션 임시)에 있었으므로 리포에 없다 — 2번에서 정식 모듈로.
